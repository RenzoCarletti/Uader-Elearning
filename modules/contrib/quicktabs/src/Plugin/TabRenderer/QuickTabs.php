...
class QuickTabs extends TabRendererBase {
  ...
  public function optionsForm(QuickTabsInstance $instance) {
    ...
    $form['permalinks'] = [
      '#type' => 'radios',
      '#title' => $this->t('Display tab permalinks'),
      '#options' => [
        TRUE => $this->t('Yes: Direct links to a quicktab within the page will display alongside the tab link.'),
        FALSE => $this->t('No: Only quicktab links will be displayed'),
      ],
      '#default_value' => ($renderer == 'quick_tabs' && $options['permalinks'] !== NULL) ? intval($options['permalinks']) : 0,
      '#description' => $this->t('Choose whether or not to display permalinks to provide shareable direct links to quicktab content alongside their respective quicktabs.</p>'),
      '#weight' => -7,
    ];
    return $form;
  }
  ...
  public function render(QuickTabsInstance $instance) {
    ...
    foreach ($instance->getConfigurationData() as $index => $tab) {
      ...
      // Transliterate and make tab name remove special characters.
      $langcode = \Drupal::languageManager()
        ->getCurrentLanguage()
        ->getId();
      $trans = \Drupal::transliteration();
      $tab_name = $trans
        ->transliterate($tab['title'], $langcode);
      ...
      $use_permalinks = isset($instance->getOptions()['quick_tabs']['permalinks']) ? $instance->getOptions()['quick_tabs']['permalinks'] : FALSE;
      if ($use_permalinks) {
        $title['1'] = Link::fromTextAndUrl(
          $this->t('Permalink'),
          Url::fromRoute(
            '<current>',
            [],
            [
              ...
              'absolute' => TRUE,
              'attributes' => [
                'class' => 'quicktabs-permalink',
                'title' => $this->t('Right-click and copy the direct link to this tab.')
              ],
            ]
          )
        )->toRenderable();
