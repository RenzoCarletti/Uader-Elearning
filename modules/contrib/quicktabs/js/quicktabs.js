...
Drupal.quicktabs.prepare = function(el) {
  ...
  $ul.find('li a.quicktabs-permalink').each(function(i, element) {
    $(element).bind('click', Drupal.quicktabs.permalinkClickHandler);
  });
  $ul.find('li a').not('.quicktabs-permalink').each(function(i, element){
...
Drupal.quicktabs.permalinkClickHandler = function(event) {
  event.preventDefault();
};
...
Drupal.behaviors.quicktabslink = {
  attach: function (context, settings) {
    ...
    $('.quicktabs-wrapper').each(function () {
      var el = $(this);
      var qt_name = Drupal.quicktabs.getQTName(el);
      var index = $.urlParam('qt-' + qt_name);
      if (index !== null) {
        var qt_target_tab = $('#quicktabs-' + qt_name + ' .quicktabs-tabs').find('[data-quicktabs-tab-name="' + index.toLowerCase() + '"]');
        if (qt_target_tab.length > 0) {
          qt_target_tab.click();
        }
      }
    });
